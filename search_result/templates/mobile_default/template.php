					...
					<?foreach($arResult['ITEM'] as $key=>$result){?>
						<?foreach($result as $stih){?>
							<?if($point==$stih['POINT']*1){?>
								<div class='search_stih'><span class='serach_stih_path' data-book='<?=$stih['BOOK']?>' data-chapter='<?=$stih['CHAPTER_NUMBER']?>' data-stix='<?=$stih['STIX_NUMBER']?>' ><a href='/mobile/bible/<?=$stih['BOOK']?>/<?=$stih['CHAPTER_NUMBER']?>/<?=$stih['STIX_NUMBER']?>/'><?=$stih['PATH']?></a></span> <?=$stih['TEXT']?></div>
								<?$arResult['COUNT']++;?>
							<?}?>
						<?}?>
					<?}?>
